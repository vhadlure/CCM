...
/*
 * Gijgo JavaScript Library v0.5.1
 * http://gijgo.com/
 *
...
gj.grid.configuration = {
    base: {
        /** The data source that is going to be used for data in rows during rendering.<br />
          ...
          * <script>
          *     var grid = $("#grid").grid({
          *         dataSource: "/version_0_5/Demos/GetPlayers",
          *         columns: [ { field: "Name" }, { field: "PlaceOfBirth" } ]
          *     });
          * </script>
          * @example <table id="grid" data-source="/version_0_5/Demos/GetPlayers">
          *     <thead>
          *         <tr>
          *             <th width="20">ID</th>
          *             <th>Name</th>
          ...
          *     };
          *     grid = $("#grid").grid({
          *         dataSource: { url: "/version_0_5/Demos/GetPlayers", data: {}, success: onSuccessFunc },
          *         columns: [ { field: "Name" }, { field: "PlaceOfBirth" } ]
          *     });
          * </script>
          * @example <table id="grid"></table>
        ...
        /** The text that is going to be displayed if the grid is empty.
          ...
          * <script>
          *     $("#grid").grid({
          *         dataSource: { url: "/version_0_5/Demos/GetPlayers", data: { searchString: "sadasd" } },
...
﻿gj.grid.events = {
    beforeEmptyRowInsert: function ($grid, $row) {
         ...
         *     var grid = $("#grid").grid({
         *         dataSource: { 
         *             url: "/version_0_5/Demos/GetPlayers",
    ...
    dataBound: function ($grid, records, totalRecords) {
         ...
         * @property {object} e - event data
         * @property {array} records - the list of records received from the server
         * @property {int} totalRecords - the number of the all records that can be presented in the grid
         * @example <table id="grid"></table>
         * <script>
         *     var grid = $("#grid").grid({
         *         dataSource: "/version_0_5/Demos/GetPlayers",
...
gj.grid.plugins.responsiveDesign = {
    'configuration': {
        ...
        defaultColumn: {
            ...
            /** The minimum width of the column. Numeric values are treated as pixels.
            ...
            if (config.columns && config.columns.length) {
                for (i = 0; i < config.columns.length; i++) {
                    result.push({
                        ...
                        minWidth: config.columns[i].minWidth || config.columns[i].width || config.minColumnWidth,
    ...
    'public': {
        ...
        responsive: function () {
            ...
            for (i = 0; i < columns.length; i++) {
                ...
                if ($column.is(':visible') && columns[i].priority && columns[i].minWidth > $column.width()) {
    ...
    'init': function ($grid) {
        ...
        var data = $grid.data('grid');
        if (data.responsive) {
            $grid.previousWidth = $grid.width();
            $grid.responsive();
            $grid.on('resize', function (e, expand) {
                $grid.responsive();
            });
            ...
            $grid.resizeCheckIntervalId = setInterval(function () {
                ...
                if (newWidth !== $grid.previousWidth) {
                    gj.grid.plugins.responsiveDesign.events.resize($grid);
                }
                $grid.previousWidth = newWidth;
            }, data.resizeCheckInterval);
        }
    }
};

$.extend(true, gj.grid.configuration.base, gj.grid.plugins.responsiveDesign.configuration);
﻿gj.grid.private = {
    ...
    SetOptions: function ($grid, jsConfig) {
        ...
        if (options.uiLibrary && options.uiLibrary === "bootstrap") {
            $.extend(true, options, gj.grid.configuration.bootstrap);
        }
        //set column configuration
        if (options.columns && options.columns.length) {
            for (i = 0; i < options.columns.length; i++) {
                column = $.extend(true, {}, options.defaultColumnSettings);
                $.extend(true, column, options.columns[i]);
    ...
    AppendEmptyRow: function ($grid, caption) {
        ...
        $cell.attr('colspan', data.columns.length);
...
gj.grid.public = {
     ...
     */
    count: function () {
        var result, $rows = $(this).find('tbody>tr');
        //exclude empty row
        if ($rows.length === 1 && $rows.data('role') === 'emptyRow') {
            result = 0;
        } else {
            result = $rows.length;
        }
     ...
     *     };
     *     grid = $("#grid").grid({
     *         dataSource: { url: "/version_0_5/Demos/GetPlayers", success: onSuccessFunc },
     *         columns: [ { field: "Name" }, { field: "PlaceOfBirth" } ]
     *     });
     * </script>
     */
